...
Maintenant que nous avons installé toutes les dépendances nécessaires, nous allons procéder à l'installation du serveur FiveM lui-même.

### Étape 1: Créer le script d'installation du serveur

Créez un nouveau script qui va télécharger et installer automatiquement la dernière version du serveur FiveM.
...
check_system() {
  log_step "🔍 VÉRIFICATIONS SYSTÈME"
  echo "================================"
  ...
  log_check "✅ Utilisateur non-root"
  ...
  . /etc/os-release
  if [[ "$ID" != "ubuntu" ]]; then
    log_warning "⚠️  OS détecté: $PRETTY_NAME (recommandé: Ubuntu 22.04 LTS)"
    read -p "Continuer quand même ? (y/N): " continue_anyway
    [[ "$continue_anyway" != "y" ]] && exit 1
  else
    log_check "✅ Ubuntu détecté: $PRETTY_NAME"
  fi
  
  ram_mb=$(free -m | awk 'NR==2{printf "%.0f", $2}')
  if [[ $ram_mb -lt 2048 ]]; then
    log_warning "⚠️  RAM: ${ram_mb}MB (recommandé: 2GB+)"
  else
    log_check "✅ RAM: ${ram_mb}MB"
  fi
  
  disk_gb=$(df -BG / | awk 'NR==2 {print $4}' | sed 's/G//')
  if [[ $disk_gb -lt 10 ]]; then
    log_warning "⚠️  Espace libre: ${disk_gb}GB (recommandé: 10GB+)"
  else
    log_check "✅ Espace libre: ${disk_gb}GB"
  fi
...
install_server() {
  ...
  # API pour récupérer la dernière version
  ...
  if [[ ! -d "~/FXServer/server-data/.git" ]]; then
...
:::tip
Ce script vérifie d'abord la compatibilité de votre système, puis télécharge et installe automatiquement la dernière version du serveur FiveM.
:::

### Étape 4: Configuration du serveur

Créez un fichier de configuration pour votre serveur FiveM.
...
sv_licenseKey "changez_moi_avec_votre_clé"
```
</details>
...
### Étape 5: Démarrer le serveur avec txAdmin

txAdmin est l'interface d'administration officielle pour les serveurs FiveM. Pour démarrer votre serveur avec txAdmin :
...
Félicitations ! Vous avez maintenant un serveur FiveM fonctionnel sur votre système Linux/Ubuntu. Dans les prochains tutoriels, nous verrons comment configurer des fonctionnalités avancées et installer des ressources supplémentaires.

:::tip Prochaines étapes
- Configuration avancée de txAdmin
- Installation de frameworks populaires (ESX, QBCore, etc.)
- Mise en place d'un système de mise à jour automatique
