...
```bash title="[command] - créer un nouvel utilisateur nommé 'fivem'"
sudo adduser fivem
```
...
```bash title="[command] - Ajoutez l'utilisateur au groupe sudo pour les permissions administratives"
sudo usermod -aG sudo fivem
```

```bash title="[command] - Connectez-vous avec le nouvel utilisateur"
su - fivem
```

:::tip title="Démonstration visuelle"
...
```bash title="[command] - Créez un nouveau fichier pour le script"
...
```bash title="[command] - Rendre le script exécutable"
chmod +x install_dependencies.sh
```

```bash title="[command] - Exécuter le script"
